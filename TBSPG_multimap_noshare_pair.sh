print_help () 
{
echo "This script automatically trim the reads, run the multi-mapping, remove the shared reads among Mitochondria, Chloroplast and Chromosomes, and sort, deduplicate the alignment map, then count the number of reads (readsno.txt) and coverage (cov.txt), using entire or half length reads."
echo "-h help"
...
echo "Example: sh TBSPG_multimap_noshare_pair.sh   -p E/H   -i READ1.fastq   -j READ2.fastq   -l Chromosome_name  -m  Mitochondria_name    -n  Chloroplast_name   -r REF_GENOME.fasta   -o OUT"
}
...
while getopts "h:p:i:j:o:r:l:n:m:" opt
do
case "$opt" in
h) print_help; exit 1
;;
...
rm -r ${outputdir}/entire_reads1_trimmed.fq ${outputdir}/entire_reads2_trimmed.fq ${outputdir}/entire_reads1_trimmed.sai ${outputdir}/entire_reads2_trimmed.sai
...
rm -r ${outputdir}/entire_multimap.sam ${outputdir}/entire_multimap_noshare.sam
...
rm -r ${outputdir}/half_reads1_trimmed.fq ${outputdir}/half_reads2_trimmed.fq ${outputdir}/half_reads1_trimmed.sai ${outputdir}/half_reads2_trimmed.sai
...
rm -r ${outputdir}/half_multimap.sam ${outputdir}/half_multimap_noshare.sam
