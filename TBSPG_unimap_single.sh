print_help () 
{
echo "This script automatically trim the reads, run the uniquely mapping, sort, and  deduplicate the alignment map, then count the number of reads (readsno.txt) and coverage (cov.txt), using entire or half length single reads."
echo "-h help"
...
echo "Example: sh TBSPG_unimap_single.sh   -p E/H   -i READ.fastq   -r REF_GENOME.fasta   -o OUT"
}
